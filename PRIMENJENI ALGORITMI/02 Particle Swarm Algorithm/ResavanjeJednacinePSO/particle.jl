...
function updateValue!(particle)
    ...
    u = particle.values[5]
    v = particle.values[6]
    particle.currentValue = 4 * x^2 - 6 * x - 3 * y ^ 3 + 0.5 * y  + 3 * z + 8 * w - 6.1 * u + 2 * v
end

function generateParticle(numberOfOperands, minRang, maxRang)
    particle = Particle(rand(minRang:0.01:maxRang, numberOfOperands), 0, 
                        fill(0.0, numberOfOperands), fill(0.0, numberOfOperands), 0)
...
function printParticle(particle)
    ...
    println(" $x $y $z $w $u $v = $(particle.currentValue)")
end
